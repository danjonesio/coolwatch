# Omarify Phase 3 — "notify" (change detection, toasts, toggles, recent.json)

Repo `/home/danjones/Projects/omarify`, base `master` at `b38379c`, branch `phase-3-notify`. Dan merges. Build with `/deej-stack:d-implement`.

## Context

Phases 1 and 2 poll Coolify and act on it; nothing tells the operator anything while the panel is closed. `docs/roadmap.md` Phase 3 turns the existing store into events: one desktop toast per real transition (deployment queued / building / deployed / failed / cancelled; a resource that stopped or degraded outside of a deployment or a user action; a server that became unreachable or reachable again), none on the baseline poll, none twice, per-event toggles under `notify{}` in the config file, click-to-open on every toast that has a Coolify page, and recent terminal deployments persisted to `~/.local/state/omarify/recent.json` so the panel's Recent section survives `omarchy restart shell`.
...
- State dir `$XDG_STATE_HOME/omarify` (fallback `~/.local/state/omarify`) created and chmod'ed `0700` by the service before the first write; the file is umask-mode (FileView has no mode API); it holds names, branches, commit messages and the instance URL, never a token.
...
```sh
omarchy-shell io.github.danjonesio.omarify status | jq '{notify, baseline, recentPersisted, recentRejected, terminalQueue, drainRetries}'
quickshell log -p /usr/share/omarchy/shell --tail 300 | grep -E '^omarify (notify|recent|drain)'   # "omarify notify finished vdyasty4"
```
...
`recent.json` at `<stateDir>/recent.json` (`stateDir = (XDG_STATE_HOME || HOME + "/.local/state") + "/omarify"`):
...
| `bin/check` | edit | `root=${OMARIFY_ROOT:-…}` override; SR16 gates over `Model.js` (launcher and notifier shape, rollback-safe) and the notifier ban in QML; SR9 QML gate untouched |
...
function _requeueDrain(uuid) {         // the one re-queue site; called from _finish (deployment kind, no successful dispatch, code ≠ 404) and the reaper
  ...
  if (n > 2) { delete root._drainTries[uuid]; console.log("omarify drain gave up " + Model.uuid8(uuid)); return }
...
- Log lines (all through `Model.uuid8`, never a name, message, path or URL): `omarify notify <event> <uuid8>` at intent; `omarify recent loaded <n>`; `omarify recent rejected`; `omarify recent save failed`; `omarify drain 404 <uuid8>`; `omarify drain gave up <uuid8>`.
...
**Verify**: `bin/check` → `ok` (qmllint resolves `qs.Commons`). `bin/dev-sync && omarchy restart shell`; `quickshell log -p /usr/share/omarchy/shell --tail 50` shows no component error and the bar icon is present (a failed `qs.Commons` import removes the service: fallback is inlining `Quickshell.execDetached(["bash","-lc",'exec "$@"',"bash"].concat(argv))`, recorded as a deviation). Within 60 s `status | jq '{baseline, notify}'` → all baseline flags true, `sentLastMin 0`, `dnd "off"`, every `suppressed` counter 0, `queued 0`; `quickshell log … | grep -c '^omarify notify'` → 0. Then, panel closed, `omarchy-shell io.github.danjonesio.omarify deploy xyhpwdxqu33omjgwuo6c7cjp` → within 5 s a "Queued <label>" or "Building <label>" toast, then "Deployed <label>" when it finishes; `grep '^omarify notify'` shows exactly those lines with an 8-char uuid and nothing else; `status | jq .notify.sentLastMin` matches; `ls ~/.local/state/omarchy/notifications/history/` gained the same number of files, each with `"app": "io.github.danjonesio.omarify"` and `execArgv[1]` beginning `https://app.coolify.io/project/`.
...
**Verify**: `bin/check` → `ok`. Pre-create `~/.local/state/omarify` at 0755 (`mkdir -m 755 -p`), then `bin/dev-sync && omarchy restart shell`; `stat -c '%a %U' ~/.local/state/omarify` → `700 danjones` (the chmod repaired it); `quickshell log … | grep 'omarify recent'` → `loaded 0` once or twice; after one deployment finishes: `jq '{version, instance, n: (.recent|length)}' ~/.local/state/omarify/recent.json` → `1`, `https://app.coolify.io`, ≥ 1; `grep -cE '[0-9]+\|[A-Za-z0-9]{20,}' ~/.local/state/omarify/recent.json` → 0; `stat -c %Y` of the file sampled every 10 s for 2 idle minutes does not change; `omarchy restart shell` → `status | jq '{recentPersisted, recentRejected, counts}'` shows the persisted count within 2 s (the load does not wait on the token); `grep -c '^omarify notify'` → 0 after the restart; `printf 'x' > ~/.local/state/omarify/recent.json` then restart → `recentPersisted 0`, `recentRejected true`, `omarify recent rejected` in the log, no error callout, no crash line; the next finished deployment rewrites a valid file; a `poll.resourcesSec` edit (a reset) followed by `status | jq .recentPersisted` shows the file was re-read.

### 8. `bin/check`: root override, SR16 gates, self-test

Files: `bin/check`.

`root=${OMARIFY_ROOT:-$(cd "$(dirname "$0")/.." && pwd)}` at `:4`. After the SR9 block, before the `--no-shell` exit: (a) launcher in `Model.js` (comments stripped): count ≤ 1 and, if 1, the line matches `"--exec", "omarchy-launch-browser", [A-Za-z_.]+\]`; (b) notifier in `Model.js`: count ≤ 1 and, if 1, the line contains `["omarchy-notification-send", `; (c) `grep -l 'omarchy-notification-send' *.qml` empty; (d) the existing SR9 QML gate untouched with its floor ≥ 1. No comment in `Model.js` may mention either program name.

**Verify**: `bin/check --no-shell` → `ok`; `d=$(mktemp -d); cp -r . "$d"` then on the copy: append `Util.execArgv(["omarchy-notification-send", "x"])` to `Service.qml` → `OMARIFY_ROOT=$d bin/check --no-shell` fails (c); revert and add a second `omarchy-launch-browser` literal to `Model.js` → fails (a); `git checkout b38379c -- Model.js` in the copy → passes (rollback-safe); `rm -rf "$d"` (the copy only, never the repo).
...
Files: none. Preconditions: `SCRATCH=/tmp/claude-1000/-home-danjones-Projects-omarify/<session>/scratchpad; mkdir -p "$SCRATCH"`; one installed build for the whole runbook; every panel closed unless a step opens one; DND off (`omarchy-shell notifications dndState` → `off`); a history snapshot `ls ~/.local/state/omarchy/notifications/history/ > "$SCRATCH/hist-N"` before each step (history is trimmed to 10 across apps); `needle() { jq -r '.instances[0].token // empty' ~/.config/omarify/config.json | cut -c1-12; }; n=$(needle); [[ -n $n ]] || { echo "needle empty (tokenCommand): run the command into the substitution instead"; exit 1; }`. Evidence channels: `quickshell log -p /usr/share/omarchy/shell --tail 300 | grep -E '^omarify (notify|recent|drain)'`; the history JSON (`jq -c '{app,summary,body,urgency,execArgv}'`); live popups as files: `ls ~/.local/state/omarchy/notifications/*.json`; the sampling loop `for i in $(seq 18); do omarchy-shell io.github.danjonesio.omarify status | jq -c '{t: now|floor, requestsLastMin, baseline, terminalQueue, counts, notify}'; sleep 10; done | tee "$SCRATCH/step-N.log"`; reduce with `jq -s 'map(.requestsLastMin) | max' "$SCRATCH/step-N.log"`. `<label>` below is `Model.appLabel` of the resource's name (for api, `xyhpwdxqu33omjgwuo6c7cjp-200537415987` elided).

1. **Silent baseline.** `bin/dev-sync && omarchy restart shell`, loop for 60 s: zero `omarify notify` lines, zero new history files, every `baseline` flag true within 10 s, `notify.dnd "off"`.
...
7. **Failed deployment (needs Dan).** Only with permission (open question 5): a temporary build-command edit in the Coolify UI, deploy, revert. Expect one `failed` line, a critical toast "Deployment failed: <label> · <dur> · click to open in Coolify", the bar alert, the toast persisting until dismissed; a still-open critical toast is replayed by the notifications plugin after a shell restart (not an Omarify re-notify: `sentLastMin` stays 0).
...
13. **Queued cancel (Phase 2 needs-human).** `d`, then `x` while the row still reads queued: record whether `GET /deployments/{uuid}` returns the deployment (→ no `cancelled` toast because rule 2 suppresses the service's own cancel; a Recent row) or 404 (→ `omarify drain 404`, nothing); fixture `deployment-cancelled.json` recorded from a real cancel if not already.
...
omarchy-shell io.github.danjonesio.omarify status | jq '{baseline, notify, recentPersisted, recentRejected, terminalQueue, drainRetries, requestsLastMin}'
quickshell log -p /usr/share/omarchy/shell --tail 300 | grep -E '^omarify (notify|recent|drain)'
...
grep -cFf <(needle) ~/.local/state/omarify/recent.json || true                             # 0
stat -c '%a %U' ~/.local/state/omarify                                                     # 700 danjones
```
