...
| runbook 13: queued cancel | IPC deploy, summon at +1.2 s, `Down x l Return` | **needs human**: the deployment was already `in_progress` 1.2 s after queuing (`GET /deployments`), so a queued cancel is not reachable on api, and the scripted keys did not dispatch a cancel (`lastAction` stayed the deploy: the first `Down` only activates the cursor). The Phase 2 queued-cancel row therefore **stays open**. What the recorded fixture does show: `af8defdknx8t5kb0kron1oaw`, cancelled 6 s after creation while already in progress, is returned by `GET /deployments/{uuid}` with `status cancelled-by-user` and `finished_at` set, so an in-progress cancel lands in Recent; a queued one is still unobserved |
