# Coolwatch Phase 2 — "act" (actions, confirm, pending, status line, keyboard map, IPC verbs)

Repo `/home/danjones/Projects/coolwatch`, base `master` at `29f3a76`, branch `phase-2-act`. Dan merges. Build with `/deej-stack:d-implement`.
...
```sh
omarchy-shell io.github.danjonesio.coolwatch stop 4kgw0…      # → queued stop 4kgw0…
omarchy-shell io.github.danjonesio.coolwatch deploy deadbeef  # → unknown uuid deadbeef   (zero requests)
omarchy-shell io.github.danjonesio.coolwatch status | jq '{lastAction, pending, pendingStale, actionsLastMin, requestsLastMin, inflightAction}'
```
...
10. **No echo.** The status line shows `elide(redact(messageOf(body)), 110)` at most; the action log line is `coolwatch action <verb> <code> exit=<n> <ms>ms <uuid8>`; `status.lastAction` is `{verb, uuid8, code, curlExit, ms, at, result}`; no name, `fqdn`, `deployment_url` or message reaches the log or IPC. Change 3, 5.
...
14. **Verification hygiene and the hard gate.** Change 0's probe must return a 403 whose message matches `/permission/i` before Change 3 or 4 is installed; otherwise the run stops and a `read`-only token is swapped in first. The token is written with `jq --rawfile`, never `--arg`. No builder or reviewer runs `bin/dev-sync` or any `--delete` tool against a real path (staging only via `COOLWATCH_DEST=$(mktemp -d)/plugin`). Change 0, 5.
...
- Idle, panel closed: `for i in $(seq 12); do omarchy-shell io.github.danjonesio.coolwatch status | jq -c '{requestsLastMin, rateLimitRemaining}'; sleep 10; done` → max. Then idle, panel open (`omarchy-shell shell summon io.github.danjonesio.coolwatch`, the loop, `hide`) → max.
- Prove the current token is `read`-only with a request that cannot mutate anything (the target is not a well-formed Coolify uuid, so it fails structurally even if the ability check passes):
  ```sh
  url=$(jq -r '.instances[0].url' ~/.config/coolwatch/config.json); tok=$(jq -r '.instances[0].token // empty' ~/.config/coolwatch/config.json)
...
2. **Token swap + real revoke.** In Coolify → Security → API Tokens create `coolwatch-act` with `read` + `deploy` only (**not** `write`). Delete the old token → bar shows the alert cloud and hero "TOKEN REJECTED" within 5 s, `status | jq .probeMode` true (closes Phase 1's "real token revoke"). Write the new token without it ever touching argv:
   ```sh
   umask 077; d=${XDG_RUNTIME_DIR:?}
   cat > "$d/tok"                                      # paste the token, Ctrl-D
   jq --rawfile t "$d/tok" '.instances[0].token = ($t | rtrimstr("\n"))' ~/.config/coolwatch/config.json > "$d/c.json"
   install -m 600 "$d/c.json" ~/.config/coolwatch/config.json; rm -f "$d/tok" "$d/c.json"
   ```
...
omarchy-shell io.github.danjonesio.coolwatch status | jq '{lastAction, pending, pendingStale, actionsLastMin, requestsLastMin, error, probeMode}'
