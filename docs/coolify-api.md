...
| GET | `/applications/{uuid}/logs` | read | `?lines=100` (max 10000) `&show_timestamps=false` → `{"logs": "<text>"}`; **400 `Application is not running.`** if no running container **[src]** |
...
| GET | `/services/{uuid}/logs?sub_service_name=<name>` | **required** `sub_service_name` = `applications[].name` or `databases[].name` from GET /services/{uuid} (not `human_name`, not container name) |
...
- No `finished_at`/duration field; use `updated_at - created_at` once status is terminal.
...
- **No log streaming**: container logs are a one-shot `docker logs --tail N` (max 10000 lines, ANSI stripped) and fail with 400 when the container isn't running; deployment logs are a JSON blob that grows — poll and diff.
